    ...
    if (!empty($_FILES['file_excel']['tmp_name'])) {
        ...
        try {
            ...
            for ($row = 2; $row <= $highestRow; $row++) { // baris 1 header
                $nis   = trim((string)$sheet->getCellByColumnAndRow(0, $row)->getValue());
                $nisn  = trim((string)$sheet->getCellByColumnAndRow(1, $row)->getValue());
                $nama  = trim((string)$sheet->getCellByColumnAndRow(2, $row)->getValue());
                $kelas = trim((string)$sheet->getCellByColumnAndRow(3, $row)->getValue());
                ...
                if ($nis != "" && $nisn != "" && $nama != "" && $kelas != "") {
                    ...
                    if (!preg_match('/^[0-9]{9}$/', $nisn)) {
                        $gagal++;
                        $errors[] = "Baris $row: NISN harus 9 digit angka.";
                        continue;
                    }
                    if (strlen($kelas) > 4) {
                        $gagal++;
